...
class CustomWeatherWidget extends StatefulWidget {
  final Size size;
  final String weather;
  final CustomSunConfig sunConfig;
  final CustomRainConfig rainConfig;
  final CustomSnowConfig snowConfig;
  final CustomCloudConfig cloudConfig;
  final CustomThunderConfig thunderConfig;
  final CustomGiftConfig giftConfig;
...
class SunWidgetState extends State<CustomWeatherWidget>
...
class CloudWidgetState extends State<CustomWeatherWidget> {
  @override
  Widget build(BuildContext context) {
    // TODO: implement build
    return Container(
...
class RainWidgetState extends State<CustomWeatherWidget>
    with TickerProviderStateMixin {
  @override
  Widget build(BuildContext context) {
    return Container(
...
class SnowWidgetState extends State<CustomWeatherWidget> {
  @override
  Widget build(BuildContext context) {
    // TODO: implement build
    return Container(
...
class CustomThunderConfig {
  final double thunderWidth;
  final Color thunderColor;

  CustomThunderConfig({this.thunderWidth, this.thunderColor});
}

class ThunderWidgetState extends State<CustomWeatherWidget> {
  @override
  Widget build(BuildContext context) {
    // TODO: implement build
    return Container(
...
class CustomGiftConfig {
  ...
  final Curve waveCurve, fadeCurve;
  final double giftSize, giftAreaYStart, giftAreaYEnd;
  final List imgUrls;
  ...
  CustomGiftConfig(
      ...
      this.giftAreaXStart,
      this.giftAreaXEnd,
      this.giftWaveRangeMin,
      this.giftWaveRangeMax,
      this.giftFallSecMin,
      this.giftFallSecMax,
      this.giftWaveSecMin,
      this.giftWaveSecMax,
...
class GiftWidgetState extends State<CustomWeatherWidget> {
  @override
  Widget build(BuildContext context) {
    // TODO: implement build
    return Container(
  ...
  _buildPaints() {
    ...
    // _paints.add(Container(width: double.infinity));

    for (int i = 0; i < widget.giftConfig.giftNum; i++) {
      String imgUrl;
      if( stringIsNotEmpty(widget.giftConfig.imgUrls) && widget.giftConfig.imgUrls.isNotEmpty){
        if(i < widget.giftConfig.imgUrls.length){
          imgUrl = widget.giftConfig.imgUrls[i];
        }else{
          int _random = Random().nextInt(widget.giftConfig.imgUrls.length);
          imgUrl = widget.giftConfig.imgUrls[_random];
        }
      }
      _paints.add(GiftWidget(
        giftAreaXEnd: widget.giftConfig.giftAreaXEnd,
        giftAreaXStart: widget.giftConfig.giftAreaXStart,
        giftAreaYEnd: widget.giftConfig.giftAreaYEnd,
        giftAreaYStart: widget.giftConfig.giftAreaYStart,
        giftColor: widget.giftConfig.giftColor,
        giftSize: widget.giftConfig.giftSize,
        giftFallSecMax: widget.giftConfig.giftFallSecMax,
        giftFallSecMin: widget.giftConfig.giftFallSecMin,
        giftWaveRangeMax: widget.giftConfig.giftWaveRangeMax,
        giftWaveRangeMin: widget.giftConfig.giftWaveRangeMin,
        giftWaveSecMax: widget.giftConfig.giftWaveSecMax,
        giftWaveSecMin: widget.giftConfig.giftWaveSecMin,
        fadeCurve: widget.giftConfig.fadeCurve,
        waveCurve: widget.giftConfig.waveCurve,
        imgUrl: imgUrl
      ));
    }
...
class GiftWidget extends StatefulWidget {
  ...
  GiftWidget(
      ...
      this.giftAreaYEnd,
      this.waveCurve,
      this.fadeCurve, this.imgUrl});

  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return SingleGiftState();
  }
}

class SingleGiftState extends State<GiftWidget> with TickerProviderStateMixin {
  @override
  Widget build(BuildContext context) {
    // TODO: implement build
    return Container(
        ...
        child: FadeTransition(
          opacity: fadeAnimation,
          child: widget.imgUrl==null ? new Icon(
            Icons.ac_unit,
            color: widget.giftColor == null ? Colors.red : widget.giftColor,
            size: widget.giftSize == null ? 20 : widget.giftSize,
          ) : Image.asset('assets/images/${widget.imgUrl}', width: widget.giftSize, height: widget.giftSize,),
        ));
  }

  AnimationController fallController, waveController, fadeController;
  // 上下移动动画、左右移动动画、淡入淡出动画
  Animation fallAnimation, waveAnimation, fadeAnimation;
  double x = 50, y = 200;
  double randomGiftX, randomGiftWave, fallStart, fallEnd;
  int randomGiftFallDuration, randomGiftWaveDuration;
  Curve fadeCurve, waveCurve;
  ...
  void initState() {
    ...
    waveCurve =
        widget.waveCurve == null ? Curves.easeInOutSine : widget.waveCurve;
    fadeCurve = widget.fadeCurve == null ? Curves.easeInCirc : widget.fadeCurve;
    fallStart = widget.giftAreaYStart == null ? 200 : widget.giftAreaYStart;
    fallEnd = widget.giftAreaYEnd == null ? 620 : widget.giftAreaYEnd;
    fallController = AnimationController(
        vsync: this, duration: Duration(seconds: randomGiftFallDuration));
    waveController = AnimationController(
        vsync: this, duration: Duration(seconds: randomGiftWaveDuration));
    fadeController = AnimationController(
        vsync: this, duration: Duration(seconds: randomGiftFallDuration));
    fallAnimation = Tween(begin: fallStart, end: fallEnd)
        .animate(CurvedAnimation(parent: fallController, curve: Curves.linear));
    waveAnimation = Tween(
            begin: randomGiftWaveDuration.isEven
                ? randomGiftX
                : randomGiftX + randomGiftWave,
            end: randomGiftWaveDuration.isEven
                ? randomGiftX + randomGiftWave
                : randomGiftX)
        .animate(CurvedAnimation(parent: waveController, curve: waveCurve));
    fadeAnimation = Tween(begin: 1.0, end: 0.0)
    ...
    fallAnimation.addStatusListener((status) {
      if (status == AnimationStatus.completed) {
        _setRandomData();
        fallController.duration = Duration(seconds: randomGiftFallDuration);
        waveController.duration = Duration(seconds: randomGiftWaveDuration);
        fadeController.duration = Duration(seconds: randomGiftFallDuration);
        fallAnimation = Tween(begin: fallStart, end: fallEnd).animate(
            CurvedAnimation(parent: fallController, curve: Curves.linear));
        waveAnimation = Tween(
                begin: randomGiftWaveDuration.isEven
  ...
  // 初始化随机位置
  _setRandomData() {
    randomGiftX = _randomGift(
            poolStart:
                widget.giftAreaXStart == null ? 30 : widget.giftAreaXStart,
            poolEnd: widget.giftAreaXEnd == null ? 220 : widget.giftAreaXEnd)
        .toDouble();
    randomGiftWave = _randomGift(
            poolStart:
                widget.giftWaveRangeMin == null ? 20 : widget.giftWaveRangeMin,
            poolEnd:
                widget.giftWaveRangeMax == null ? 110 : widget.giftWaveRangeMax)
        .toDouble();
    randomGiftFallDuration = _randomGift(
        poolStart: widget.giftFallSecMin == null ? 10 : widget.giftFallSecMin,
        poolEnd: widget.giftFallSecMax == null ? 60 : widget.giftFallSecMax);
    randomGiftWaveDuration = _randomGift(
        poolStart: widget.giftWaveSecMin == null ? 5 : widget.giftWaveSecMin,
        poolEnd: widget.giftWaveSecMax == null ? 20 : widget.giftWaveSecMax);
  }
  // 指定范围得到一个随机数
